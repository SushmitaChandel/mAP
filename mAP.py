...
def detection_false_alarm_rates(
	pred_boxes, true_boxes, iou_threshold = 0.5, box_format="corners",
	num_classes = 20
 ):
	...
	for c in range(num_classes):
		...
		if total_true_bboxes == 0:
			...
			detection_rate = TP_sum / (total_true_bboxes + epsilon)
		if TP_sum == 0 and FP_sum == 0:
			false_alarm_rate = 0
		else:
			false_alarm_rate = FP_sum / (TP_sum + FP_sum + epsilon)
		
		detection_rates.append(detection_rate)
		false_alarm_rates.append(false_alarm_rate)

	return detection_rates,false_alarm_rates